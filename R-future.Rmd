...
system.time(for(i in seq_len(n)) {
     out[[i]] <- future( {
       ## simply insert code here as in foreach; for example:
       tmp <- rnorm(1e8)
       c(mean(tmp), sd(tmp))
     })
})
## Return value ('out') is a wrapper, so use value() to access:
system.time(value(out[[1]]))
system.time(value(out[[20]]))
```
...
Note it's not possible to do this with `parLapply` as it requires passing a cluster object, but something like this would be possible with `mclapply`.
