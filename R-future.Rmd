...
# check if the calculation is done in a non-blocking way:
system.time(resolved(futureOf(out)))
# getting the value is blocking
system.time(out)
```
...
Note that I use `future.seed = 1` above (rather than `future.seed = TRUE` as previously) in this case as a demonstration of how to get results that are the same every time the code is run. Of course you can use a number other than 1. 
