...
The `sequential` plan allows you to run code on a single local core. This might not seem all that useful since the goal is to parallelize, but this helps in debugging and allows someone to run future-based code even if they only have one core available.
...
```{r, cluster2}
workers <- c(rep('arwen', 2), rep('beren', 2))
plan(cluster, workers = workers)
...
On the SCF it is fine. Here's an example. Note that the `resources` argument tells what the SLURM arguments should be for *each* worker.
...
output <- future_sapply(1:100, function(i) mean(rnorm(1e7)))
```
