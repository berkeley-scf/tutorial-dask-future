...
```{r, plan}
plan(multisession)  ## spreads work across multiple cores
...
The deprecated `multiprocess` plan used either `multisession` on Windows and `multicore` on MacOS/Linux.
...
Alternatively, you can use `furrr:future_map` to run a parallel map operation explicitly (i.e., taking a functional programming perspective).
...
(Here the code is safe in terms of parallel randon number generation because of the `seed` argument - see section later in this document.)
...
(Note that, as warned, the code here is not safe in terms of parallel randon number generation - see section later in this document.)
...
# check if the calculation is done in a non-blocking way:
...
If we know the names of the machines and can access them via password-less SSH (e.g., [using ssh keys](https://statistics.berkeley.edu/computing/ssh-keys), then we can simply provide the names of the machines to create a cluster and use the 'cluster' plan.
...
If you are using SLURM and in your sbatch or srun command you use `--ntasks`, then the following will allow you to use as many R workers as the value of `ntasks`. One caveat is that one still needs to be able to access the various machines via password-less SSH.
...
On non-Windows machines, the multicore plan (not available on Windows or in RStudio) forks the main R process. This creates
...
Unfortunately, the *top* program on Linux or MacOS will make it look like additional memory is being used. On a Linux machine the command `free -h` shows the usable remaining memory under the 'Available' column of the 'Mem' row, e.g., here we see 6.5 GB available and 8.2 GB used. (Note that the 'free' column omits memory that is actually available but is temporarily in use for caching.)
...
See the [future vignette on topologies](https://cran.r-project.org/web/packages/future/vignettes/future-3-topologies.html) for more configuration examples.

The really nice thing about this is that once you've set your code to allow parallelization at multiple levels, you can change the plan without ever touching the core code again.
...
The future package [integrates well with the L'Ecuyer parallel RNG approach](https://www.jottr.org/2020/09/22/push-for-statical-sound-rng/#random-number-generation-in-the-future-framework). There is a good discussion about seeds for `future_lapply` and `future_sapply` in the help for those functions.
...
I could also have set `future.seed = 1` instead of setting the seed using `set.seed`.
...
With implicit futures, you'll need to manually set the seed to a L'Ecuyer-CMRG seed, which is a vector of six integers. Here's how you can do it, advancing the seed for each iteration using nextRNGStream().
